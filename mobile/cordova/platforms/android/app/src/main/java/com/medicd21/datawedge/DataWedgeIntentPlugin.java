...
/**
 * Simple Cordova plugin that registers a BroadcastReceiver for DataWedge intents
...
public class DataWedgeIntentPlugin extends CordovaPlugin {
    ...
    private void registerReceiver() {
        ...
        // DataWedge typically broadcasts with these actions; listen broadly
        ...
        receiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                // DataWedge may provide several payload keys; try common ones
                String tag = null;
                ...
                if (tag == null) {
                    // sometimes DataWedge nests data in a map; attempt to stringify
                    try {
                        if (extras.size() > 0) {
                            tag = extras.toString();
                        }
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        if ("register".equals(action)) {
            // already registered on initialize
